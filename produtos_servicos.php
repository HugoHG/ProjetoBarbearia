...
<head>
    ...
    <script>
        ...
        function mudarDesign(d){
            if(d == 2){
                window.location.replace("produtos_servicos.php?design=css/style2.css");
            } 
            else if(d == 1){
                window.location.replace("produtos_servicos.php?design=css/style.css");
            }
    ...
    <div id="main">
        ...
        <div id="abaixodoheader">
            ...
            <div id="contentbarbearia">
                ...
                $sql = 'select * from tbl_destaque where idEstabelecimento = '.$estabelecimentoPag;

                $result = mysqli_query($conexao, $sql);

                while ($item = mysqli_fetch_array($result)){
                    $caminhoimagem = str_replace("../", "", $item['imagem']);
                    ?>
                    <div class="prod_destaque">
                ...
                ?>
                <button class="botaopassaresquerda" onclick="plusDivs(-1)">&#10094;</button>
                <button class="botaopassardireita" onclick="plusDivs(1)">&#10095;</button>
            </div>
        </div>
        <script>
            var slideIndex = 1;
          ...
          function showDivs(n) {
            var i;
            var x = document.getElementsByClassName("prod_destaque");
            if (n > x.length) {
                slideIndex = 1;
            }    
            if (n < 1) {
