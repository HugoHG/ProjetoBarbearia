    ...
    <script>
        ...
        function showSlides(n) {
            ...
            function mudarDesign(d){
                if(d == 2){
                    window.location.replace("promocoes.php?design=css/style2.css");
                } else if(d == 1){
                    window.location.replace("promocoes.php?design=css/style.css");
                }
        ...
        <div id="main">
            ...
            <div id="abaixodoheader">
                ...
                <div id="contentbarbearia">
                    ...
                    $sql = "select * from view_promocoes where visibilidade = 1 and idEstabelecimento = ".$estabelecimentoPag;

                    $resultadoSelect = mysqli_query($conexao, $sql);

                    while ($item = mysqli_fetch_array($resultadoSelect)){
                        $caminhofoto = str_replace("../", "", $item['foto']);
                        $precoreal = $item['valor'] - ($item['desconto'] * $item['valor']/100);
                        ?>
                        <div class="produto3">
                            <img class="imgproduto" src="<?php echo($caminhofoto) ?>" alt="Corte de cabelo">
                            <p>Nome:<?php echo($item['nomeProduto']) ?></p>
                            <p>Descrição:<?php echo($item['descProduto']) ?></p>
                            <p>Preço: de <span style="text-decoration: line-through;"> <?php echo($item['valor']) ?> </span> por R$: <?php echo($precoreal) ?></p>
